...
def _init_session(session=None, **kwargs):
    ...
    if session is None:
        ...
        if (not isinstance(session, FuturesSession)) and (len(session.cookies.values())==0):
            #: Make a call for getting cookies
            first_response = session.get("https://fc.yahoo.com")
            cookies_str = get_cookie_header(first_response.cookies, first_response)
        elif (not isinstance(session, FuturesSession)) and (len(session.cookies.values())==1):
            cookies_str = session.cookies.values()[0]
        else:
            cookies_str = ""
